...
public extension UIApplication {
    ...
    func setupFullAppearance(_ tintColor: UIColor, barTintColor: UIColor) {
       ...
       _ = {
           $0.isUserInteractionEnabled = true
           //$0.font = UIFont(name: fontName, size: 11)
         }(UILabel.appearance())
    ...
    @available(iOS 13.0, *)
    var detectIsDeviceDarkModeEnabled: Bool {
    ...
    var sdkCurrentKeyWindow: UIWindow? {
     ...
     func sdkTopViewController(_ base: UIViewController? = UIApplication.shared.keyWindow?.rootViewController) -> UIViewController? {
        ...
        if let navigationController = base as? UINavigationController {
            return sdkTopViewController(navigationController.visibleViewController)
        }
          
        if let tabBarController = base as? UITabBarController {
            if let selected = tabBarController.selectedViewController {
                return sdkTopViewController(selected)
            }
        }
          
        if let presented = base?.presentedViewController {
            return sdkTopViewController(presented)
        }
        return base
    }
    ...
    func didEnterBackground(_ block: (()->Void)? = nil) {
        ...
        backgroundQueue.addOperation() {
            NSLog("SDK some work in background")
            block?()
            application.endBackgroundTask(bgTask)
            bgTask = UIBackgroundTaskIdentifier.invalid
        }
