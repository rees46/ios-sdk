      ...
      self.getRequest(path: path, params: params) { result in
        ...
        case let .success(successResult):
          do {
            ...
            let resultResponse = try decoder.decode(RecommenderResponse.self, from: jsonData)
            completion(.success(resultResponse))
          } catch {
            completion(.failure(SdkError.decodeError))
          }
        case let .failure(error):
          completion(.failure(error))
        }
      ...
      self.getRequest(path: path, params: params) { result in
        ...
        case let .success(successResult):
          do {
            ...
            let resultResponse = try JSONDecoder().decode(ProductsListResponse.self, from: jsonData)
    ...
    sessionQueue.addOperation {
      ...
      self.getRequest(path: path, params: params) { result in
        ...
        case let .success(successResult):
          do {
            let jsonData = try JSONSerialization.data(withJSONObject: successResult, options: [])
            let resultResponse = try JSONDecoder().decode(ProductInfo.self, from: jsonData)
            completion(.success(resultResponse))
          } catch {
            completion(.failure(SdkError.decodeError))
          }
        ...
        case let .success(responseJson):
          do {
            guard let data = responseJson["data"] as? [String: Any],
                  let itemsJSON = try? JSONSerialization.data(withJSONObject: data["items"] as Any, options: [])
            else {
            ...
            let decoder = JSONDecoder()
            let items = try decoder.decode([CartItem].self, from: itemsJSON)
      ...
      self.getRequest(path: path, params: params, false) { result in
        ...
        case let .success(successResult):
          do {
            let jsonData = try JSONSerialization.data(withJSONObject: successResult, options: [])
            let decoder = JSONDecoder()
            let res = try decoder.decode(StoryContent.self, from: jsonData)
