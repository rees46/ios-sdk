...
struct ShopCartDataService {
    static var instance = ShopCartDataService()
    ...
    mutating func addProduct(toCart product: ShopCartPrepareProduct, numberOfProduct: Int, status: Bool) {
    ...
    mutating func removeProduct(fromCart product: ShopCartPrepareProduct?, selectedProduct:  ShopSelectedProduct?) {
    ...
    func isAddedToCart(_ product: ShopCartPrepareProduct) -> String {
    ...
    func isProductInCart(_ product: ShopCartPrepareProduct) -> Bool {
        for (_, value) in cart.products.enumerated() {
            if value.selectedProduct.title == product.title && value.addToCart == true {
                return true
            }
    ...
    mutating func updateQuantity(incart withItem:  ShopSelectedProduct?,optionalItem: ShopCartPrepareProduct? = nil , with quantity: Int) {
